  ...
  return (
    <div className="max-w-6xl mx-auto space-y-5 pb-12 pt-2 px-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-[22px] font-bold text-slate-800 tracking-tight">Recurring Invoices</h1>
          <p className="text-[13px] text-slate-500">Automate repeating billing cycles</p>
        </div>
      ...
      {showForm && (
        <Card>
          <CardHeader><CardTitle className="text-base">New Recurring Invoice</CardTitle></CardHeader>
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div><label className="text-xs text-slate-500 mb-1 block">Customer Name *</label><Input value={form.customer_name} onChange={e=>setForm(f=>({...f,customer_name:e.target.value}))} placeholder="Customer name"/></div>
              ...
              <div className="md:col-span-2"><label className="text-xs text-slate-500 mb-1 block">Description</label><Input value={form.description} onChange={e=>setForm(f=>({...f,description:e.target.value}))} placeholder="Invoice description"/></div>
            </div>
      ...
      <div className="relative max-w-xs">
        ...
        <Input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search recurring invoices..." className="pl-9"/>
      </div>
                  ...
                  {filtered.map((r: any) => {
                    ...
                    return (
                      <tr key={r.id} className="border-b border-slate-50 hover:bg-slate-50">
                        ...
                        <td className="px-4 py-3 text-emerald-600 font-medium">₹{Number(r.amount).toLocaleString()}</td>
                        <td className="px-4 py-3 text-slate-600 capitalize">{FREQ_LABELS[r.frequency]||r.frequency}</td>
                        <td className="px-4 py-3 text-slate-500">{r.next_due || r.start_date}</td>
                        ...
                        <td className="px-4 py-3">
                          {r.status !== 'cancelled' && (
                            <Button variant="ghost" size="sm" className="text-xs text-slate-500 hover:text-slate-800 h-7 px-2" onClick={()=>toggleStatus(r.id,r.status)}>
